...
# First version: model that uses listeners' Openface features only + model that uses speakers' audio features only
class DoubleSEMPI(nn.Module):
    def __init__(self, config):
        ...
        self.out_speaker = Classifier(in_size=clf_size, hidden_size=config.hidden_size, 
                              dropout=config.dropout, num_classes=config.num_labels, config = config)
        self.out_listener = Classifier(in_size=clf_size, hidden_size=config.hidden_size, 
                              dropout=config.dropout, num_classes=config.num_labels, config = config)
    ...
    def forward(self, audio_paths, openfacefeat_):
        ...
        #speaker = self.out_speaker(openfacefeat)
        speaker = self.out_speaker(openfacefeat_filetered)
        listener = self.out_listener(audio_features)


        e = self.mlp(torch.cat([speaker, listener], dim = 1))

        return e

# Second version: two unimodal models + cross attention module
class FacialAudioFusion(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
    ...
    def forward(self, audio_paths, openfacefeat_):
        ...
        # Process audio features to get intermediate representation
        audio_features = self.audio_enc.extract_feats(audio_paths).view(B, -1)
        audio_intermediate = self.audio_mlp(audio_features) # B x hidden_size

        # Process OpenFace features to get intermediate representation
        openfacefeat =  [i.mean(0) for i in openfacefeat_]
        openfacefeat = torch.stack(openfacefeat)
        ...
        # Filter the OpenFace features based on configuration
        openfacefeat_filtered = openfacefeat[:, [OPENFACE_COLUMN_NAMES.index(i) for i in self.filteredcolumns]]
        facial_intermediate = self.facial_mlp(openfacefeat_filtered) # B x hidden_size

        # Path 1: Concatenation of intermediate features followed by MLP
        concat_features = torch.cat([facial_intermediate, audio_intermediate], dim=1) # [B, hidden_size*2]
        concat_output = self.fusion_mlp(concat_features) # [B, hidden_size]

        # Path 2: Cross-attention between facial and audio features
        # Reshape for attention (seq_len, batch, hidden_dim)
        facial_attn = facial_intermediate.unsqueeze(0)  # 1 x B x hidden_size
        audio_attn = audio_intermediate.unsqueeze(0)  # 1 x B x hidden_size
        ...
        attn_output = attn_output.squeeze(0)  # B x hidden_size

        # Combine outputs from both paths
        combined_output = concat_output + attn_output
